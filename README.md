...
The '<.+>' operator does more behind the scenes than simply store tokens. If you want to use a normal Haskell list to hold your tokens, however, you can; just use the 'regexFromList' function.

All that function really does is apply the '<.+>' operator to each item in the list with a foldr. The output will, thus, always be the same.
